...
// ----------------------------------------------------------------------------
// scope of algorithm is a sentence containing an NegatableNamedEntityMention
// ----------------------------------------------------------------------------
Collection<Sentence> sents = select type:Sentence, filter:contains(NegatableNamedEntityMention)
...
// ----------------------------------------------------------------------------
select(type:NegationScope, filter:contains(NegatableNamedEntityMention)).each { NegationScope scope ->
    select(type:PreNegationTrigger, filter:coveredBy(scope)).each { PreNegationTrigger trigger ->
        select(type:NegatableNamedEntityMention,
        filter:and(coveredBy(scope), after(trigger.end))).each { NegatableNamedEntityMention mention ->
            mention.polarity = -1
        }
    }
    select(type:PostNegationTrigger, filter:coveredBy(scope)).each { PostNegationTrigger trigger ->
        select(type:NegatableNamedEntityMention,
        filter:and(coveredBy(scope), before(trigger.begin))).each { NegatableNamedEntityMention mention ->
            mention.polarity = -1
        }
    }
}
